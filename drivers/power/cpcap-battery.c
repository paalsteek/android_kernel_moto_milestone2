...
static void cpcap_fix_charge_count(struct cpcap_batt_ps *sply) {
	if (sply->batt_state.batt_volt >= 4100000 &&  99 == sply->batt_state.batt_capacity_one &&
		0 == sply->usb_state.online && 0 == sply->ac_state.online) {
				   ...
				   union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		val->intval = sply->bat_max_volt;
		break;

	case POWER_SUPPLY_PROP_VOLTAGE_MIN:
		val->intval = sply->bat_min_volt;
		break;
...
static int cpcap_batt_probe(struct platform_device *pdev)
{
	...
	// Engle, adjust the max voltage
	sply->bat_max_volt = 4200000;
	sply->bat_min_volt = 3500000;
